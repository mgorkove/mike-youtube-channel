...
def compute_publish_schedule(
    ...
    When *same_day* is ``False`` (default), scheduling starts from the
    next Monday after today.  When ``True``, scheduling starts today —
    any slots whose time has already passed are still included (YouTube
    will publish them immediately).
    ...
    same_day:
        If ``True``, start scheduling from today instead of next Monday.
    """
